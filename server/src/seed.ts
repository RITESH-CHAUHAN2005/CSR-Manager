...
export async function seedDatabase() {
  ...
  // --- Projects ---
  // Each seeded project is funded by a single company that pledged its whole budget,
  // so `commitments` sums exactly to `budget` (no shortfall/excess on seed data).
  const [p1, p2, p3, p4, p5, p6, p7] = await Project.create([
    { name: 'Digital Literacy Program', companyIds: [tcs._id], commitments: [{ companyId: tcs._id, committedAmount: 2500000 }], derivedStatus: 'other', startDate: '2022-04-15', endDate: fy1.endDate, category: 'Education', location: 'Pune, Maharashtra', budget: 2500000, status: 'completed', description: 'Training rural youth in digital skills and computer usage' },
    { name: 'Clean Water Initiative', companyIds: [tcs._id], commitments: [{ companyId: tcs._id, committedAmount: 3500000 }], derivedStatus: 'other', startDate: '2023-05-10', endDate: fy2.endDate, category: 'Environment', location: 'Nagpur District', budget: 3500000, status: 'completed', description: 'Installation of water purification systems in 10 villages' },
    { name: 'Women Empowerment Program', companyIds: [tcs._id], commitments: [{ companyId: tcs._id, committedAmount: 4000000 }], derivedStatus: 'ongoing', startDate: '2024-04-20', endDate: '2029-03-31', category: 'Skill Development', location: 'Nashik, Maharashtra', budget: 4000000, status: 'active', description: 'Skill development for rural women entrepreneurs' },
    { name: 'BridgeIT Scholarship', companyIds: [infosys._id], commitments: [{ companyId: infosys._id, committedAmount: 1800000 }], derivedStatus: 'other', startDate: '2023-07-01', endDate: fy2.endDate, category: 'Education', location: 'Bengaluru, Karnataka', budget: 1800000, status: 'completed', description: 'Scholarships for underprivileged students in STEM' },
    { name: 'Rural Healthcare Camp', companyIds: [infosys._id], commitments: [{ companyId: infosys._id, committedAmount: 2200000 }], derivedStatus: 'ongoing', startDate: '2024-06-10', endDate: '2029-03-31', category: 'Healthcare', location: 'Mysuru District, Karnataka', budget: 2200000, status: 'active', description: 'Free healthcare services and medicine distribution' },
    { name: 'Tree Plantation Drive', companyIds: [hdfc._id], commitments: [{ companyId: hdfc._id, committedAmount: 1000000 }], derivedStatus: 'other', startDate: '2023-08-05', endDate: fy2.endDate, category: 'Environment', location: 'Thane, Maharashtra', budget: 1000000, status: 'completed', description: 'Large-scale tree plantation and green cover initiative' },
    { name: 'Old Age Care Center', companyIds: [hdfc._id], commitments: [{ companyId: hdfc._id, committedAmount: 2000000 }], derivedStatus: 'ongoing', startDate: '2024-07-15', endDate: '2029-03-31', category: 'Healthcare', location: 'Mumbai, Maharashtra', budget: 2000000, status: 'active', description: 'Care and support center for senior citizens' },
  ])
